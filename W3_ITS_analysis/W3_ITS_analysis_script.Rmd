...
I used software called 'Cutadapt' employed in linux environmnet with python. First I removed reverse-complement primer parts as: 
...
for ((i=0; i<${#for[*]}; i++)) # iterates over the forward reads array
  ...
  echo $trimmed_for
  cutadapt -a GCATCGATGAAGAACGCAGC -A TTACTTCCTCTAAATGACCAAG --no-indels -o $trimmed_for -p $trimmed_rev ${for[i]} ${rev[i]} -m 1 --cores 5   done
```
...
```{R eval=FALSE}
  ...
  for ((i=0; i<${#for[*]}; i++)) # iterates over the forward reads array
    ...
    echo $trimmed_for
    cutadapt -a TACTGACTGACT -A GCCTGCTCGACG --no-indels -o $trimmed_for -p $trimmed_rev ${for[i]} ${rev[i]} -m 1 --cores 5 
    done
```  
...
fnFs <- sort(list.files(path, pattern="_R1_trim_trim.fastq", full.names = TRUE)) #sort forward and reverse files
fnRs <- sort(list.files(path, pattern="_R2_trim_trim.fastq", full.names = TRUE)) #sort forward and reverse files
...
filtFs <- file.path(path, "filtered", paste0(sample.names, "_F_filt.fastq.gz")) # Place filtered files in filtered/ subdirectory
...
Filtering low quality of sequence is done and now we merge pair-end reads. Chimera can occur duing merging, therefore remove them. 
...
Track the number of reads reads through the pipeline. By looking at it, you will see if you lost too many reads in which step.
...
```{r eval=FALSE}
taxa <- assignTaxonomy(seqtab.nochim,"~/Ricebiome/rice_sequencing_process/Rice_ITS_Bora/ITS_trim_primer_3end/ITS_trim_adapter_3end/tax/sh_general_release_dynamic_04.02.2020.fasta", multithread=TRUE)
```
...
ps <- phyloseq(otu_table(seqtab.nochim, taxa_are_rows=FALSE), tax_table(taxa)) #incorporate all datasets into phyloseq object to handle easier
...
```{r eval=FALSE}
SAM=sample_data(meta, errorIfNULL = T) #add metadata (that is same one used in W1. phenotype data) into phyloseq object
...
fun_alpha<-as.data.frame(cbind(shannon, no.species, chao1, evenness, sample_data(ps2)))
...
fun_alpha_summary_soil_com<-as.data.frame(t(st2[,-(5:6)])) 
...
for(i in 1:np2){
  a<-aggregate(phyla_test_perc[,i], by=list(Soil_compartment=phyla_test_perc$Soil_compartment), FUN=sum) #aggregate by soil_com
  phyla_soilcom[,i]<-a$x
}
...
phyla_soilcom_rc_melt$phylum<-factor(phyla_soilcom_rc_melt$rowname, 
                                     c("Ascomycota","Basidiomycota","Mortierellomycota","Chytridiomycota",
                                       "Olpidiomycota","Mucoromycota","Glomeromycota","Rozellomycota","Others","Unknown"))
...
p + scale_fill_manual(values=cols) + theme_classic() + 
  theme(text = element_text(size=18), axis.text.x = element_text(angle=90, hjust=1, colour = "black"))+ labs(x="")
```
...
t_glome_order_sum$Compartment2<-factor(t_glome_order_sum$Compartment,c("Bulksoil","Rhizosphere","Root"))
...
for(i in 1:p){
  a<-aggregate(t_glome_order_sum[,i], by=list(Soil_compartment=t_glome_order_sum$Soil_compartment), FUN=sum)
...
glome_order_soilcom_rc_melt$Order<-factor(glome_order_soilcom_rc_melt$rowname, 
                                     c("Archaeosporales","Diversisporales","Glomerales","Paraglomerales",
                                       "Gigasporales","Unknown"))

cols<-c("#a6cee3", "#1f78b4", "#b2df8a", "#33a02c","#808000", "#D3D3D3") #assign colors
p<-ggplot(glome_order_soilcom_rc_melt, aes(variable, value, fill=Order)) + geom_bar(stat="identity", position="fill") 
p + scale_fill_manual(values=cols) + theme_classic() + 
  theme(text = element_text(size=18), axis.text.x = element_text(angle=90, hjust=1, colour = "black"))+ labs(x="")
...
```{r echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
shapiro.test(resid(glm.nb(Archaeosporales ~ total_fresh_biomass_g_per_plant, FoRT_glome_order)))[2]$p.value 
shapiro.test(resid(glm.nb(Diversisporales ~ total_fresh_biomass_g_per_plant, FoRT_glome_order)))[2]$p.value 
shapiro.test(resid(glm.nb(Archaeosporales ~ total_fresh_biomass_g_per_plant, FoRS_glome_order)))[2]$p.value 
shapiro.test(resid(glm.nb(Glomerales ~ total_fresh_biomass_g_per_plant, FiRS_glome_order)))[2]$p.value
...
As SLs were only detected in the plant roots grown on forest soil, all correlation study with SLs performed using forest soil dataset. 
...
for(i in 1:vars) {
  l<-lmp(match_alpha[,i]~X4DO_pmol_g, data=match_alpha)
...
for(i in 1:var) {
  RT.p[i]<-(anova.cca(capscale(match_Fo_RT[14:2840]~SLs_Fo_RT[,i], match_Fo_RT, dist="bray"), step=1000))$`Pr(>F)`[1]
  RS.p[i]<-(anova.cca(capscale(match_Fo_RS[14:2840]~SLs_Fo_RS[,i], match_Fo_RS, dist="bray"), step=1000))$`Pr(>F)`[1]
}
...
match_Fo_RT_filt<-as.data.frame(match_Fo_RT_filt[,colSums(match_Fo_RT_filt[,])>2*(dim(match_Fo_RT_filt)[1])]) 
...
match_Fo_RS_filt<-as.data.frame(match_Fo_RS_filt[,colSums(match_Fo_RS_filt[,])>2*(dim(match_Fo_RS_filt)[1])])
...
RT_major_genus<-as.data.frame(t(subset(RT_genus_sum, percentage >=0.3))) #select genera which are abundant >1%
...
RS_major_genus<-as.data.frame(t(subset(RS_genus_sum, percentage >=0.1))) #select genera which are abundant >0.5%
